...
export class CreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.createFormGroup = this._fb.group({
      ...
      emailGroup: this._fb.group(
        {
          Email: ["", [Validators.required, CustomValidator.emailDomain('dell.com')]], // our custom validator class
    ...
    this.createFormGroup.get('contactPreference').valueChanges.subscribe(data => {
  ...
  bindEmployeeToForm(employee: IEmployee){
    ...
    this.createFormGroup.setControl('skills', this.setSkillsFormArray(employee.skills)) // using setControl method to bind FormArray to html Form
  }
  ...
  addSkillButtonClick(): void {
    ...
    (<FormArray>this.createFormGroup.get("skills")).push(this.addSkillsFormGroup()); // passing function of creating FormGroup control
  }
  ...
  setRequiredProperty(contact: string) {
    ...
    const confirmEmailControl = this.createFormGroup.get('emailGroup').get("confirmEmail");
    if (contact == 'phone') {
  ...
  updateEmployeeInfo(){
    ...
    this.updatedEmployee.contactPreference = this.createFormGroup.value.contactPreference;
...
function EmailMatch(group: AbstractControl): { [key: string]: any } | null {
  ...
  if (emailControl.value.toUpperCase() === confirmEmailControl.value.toUpperCase()
         || (confirmEmailControl.pristine && confirmEmailControl.value === "" /*the user didnt has the chance to write inside it */)) {
    return null;
  } else {
