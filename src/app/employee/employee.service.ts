...
export class EmployeeService{
    ...
    getEmployees(): Observable<IEmployee[]>{
        return this._http.get<IEmployee[]>(this.baseUrl).pipe(catchError(this.errorHandler))
    }

    getEmployee(employeeId: number): Observable<IEmployee>{
        return this._http.get<IEmployee>(`${this.baseUrl}/${employeeId}`).pipe(catchError(this.errorHandler))
    }
    ...
    addEmployee(employee: IEmployee): Observable<void>{
        return this._http.post<void>(`${this.baseUrl}`, employee, {
            headers: new HttpHeaders({
                'Content-Type': 'application/json'
            })
        }).pipe(catchError(this.errorHandler))
